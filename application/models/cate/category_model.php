...
class Category_model extends CI_Model{
	...
	public function list_cate($pid = 0){
		...
		$query = $this->db_connect2->get(self::TBL_CATE);
	...
	public function add_category($data){
		return $this->db_connect2->insert(self::TBL_CATE,$data);
	}

	#获取单条信息
	public function get_cate($cat_id){
		$condition['cat_id'] = $cat_id;
		$query = $this->db_connect2->where($condition)->get(self::TBL_CATE);
	...
	public function update_cate($data,$cat_id){
		...
		$condition['cat_id'] = $cat_id;
		return $this->db_connect2->where($condition)->update(self::TBL_CATE,$data);
	}

	#删除
	public function delete_cate($cat_id){
		$condition['cat_id'] = $cat_id;
		$query = $this->db_connect2->where($condition)->delete(self::TBL_CATE);
